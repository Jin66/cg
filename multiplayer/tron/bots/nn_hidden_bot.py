...
class NNHiddenBot(AbstractBot):
    width = 30
    height = 20
    can_lose_stupidly = False
    input_modes: List[InputMode] = []

    def __init__(self, weights=None, can_lose_stupidly=False, input_modes=None, size_hidden=4):
    ...
    def get_init_input(self, init):
        ...
        if self.weights is None:
            self.weights_input = np.random.uniform(low=-2, high=2, size=(size_input, self.size_hidden))
            self.weights_hidden = np.random.uniform(low=-2, high=2, size=(self.size_hidden, 4))
        else:
            number_weights_input = size_input * self.size_hidden
            self.weights_input = np.reshape(self.weights[:number_weights_input], (size_input, self.size_hidden))
            self.weights_hidden = np.reshape(
                self.weights[number_weights_input:number_weights_input + self.size_hidden * 4],
                (self.size_hidden, 4))
    ...
    def get_next_play(self):
        ...
        # Compute outputs
        hidden = self.sigmoid(np.dot(inputs, self.weights_input))
        outputs = self.sigmoid(np.dot(hidden, self.weights_hidden))
