...
class Board:
    ...
    def clean(self, id_bot):
        ...
        for cell in cells:
            self.neighbors[cell] = self.accessible_neighbors(cell)
    ...
    def update_accessible_neighbors(self, pos):
        debug = tuple([17, 11])
        if debug == pos:
            print(self.neighbors[pos], file=sys.stderr)
        for neighbor in self.accessible_neighbors(pos):
...
class BasicBot:
    ...
    def get_next_play(self):
        ...
        if self.debug:
            print(close_cells, file=sys.stderr)
        ...
        if self.debug:
            print(score, " ", min(score, key=score.get), file=sys.stderr)
...
while True:
    ...
    print(bot.board.neighbors[tuple([18, 11])],
          bot.board.neighbors[tuple([19, 11])],
          bot.board.neighbors[tuple([20, 11])],
          bot.board.neighbors[tuple([21, 11])],
          bot.board.neighbors[tuple([22, 11])],
          file=sys.stderr)
    if turn == 149:
        print(bot.board.multiple_accessible_path([18, 11]), file=sys.stderr)
